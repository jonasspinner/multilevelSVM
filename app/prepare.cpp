...
void read_csv(const std::string & filename, MyMat & min_data, std::vector<int> & maj_data, int label_col = 0, const std::string & label_min = "-1");
...
int parse_args(int argc, char *argv[], config & conf) {
        ...
        struct arg_int *nearest_neighbors   = arg_int0(nullptr, "nn", nullptr, "Number of nearest neighbors to compute. (default 10)");
        struct arg_int *label_column        = arg_int0(nullptr, "label_col", nullptr, "column in which the labels are written (starting at 0)");
        struct arg_str *label_minority      = arg_str0(nullptr, "minority", nullptr, "label/class of the minority class for binary classifications (default \"1\")");
        struct arg_lit *p_csv               = arg_lit0("c", nullptr, "export the csv where the categorical attributes where converted to binary");
        ...
        struct arg_str *file_format         = arg_str0(nullptr, "file_format", "[csv|libsvm]", "The format of the input file (default behavior is csv if the file ends with \".csv\")");
	...
	if (file_format->count > 0) {
        ...
        std::cout << csv_str << " compare " << file_format->sval[0] << " = " << (csv_str == file_format->sval[0]) << std::endl;
...
void read_csv(const std::string & filename, MyMat & data, std::vector<int> & labels, int label_col, const std::string & label_min) {
        enum COL_TYP {
                LABEL,
                NUMERICAL,
                CATEGORICAL
