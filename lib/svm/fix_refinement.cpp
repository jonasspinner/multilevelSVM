...
template <class T>
fix_refinement<T>::fix_refinement(graph_hierarchy &min_hierarchy, graph_hierarchy &maj_hierarchy,
...
template <class T> svm_result<T> fix_refinement<T>::step(const svm_data &min_sample, const svm_data &maj_sample) {
...
template <class T>
svm_result<T> fix_refinement<T>::train_fix(svm_solver<T> &solver, const svm_data &min_sample,
                                           const svm_data &maj_sample, float C, float gamma) {
    svm_summary<T> summary = solver.train_single(std::make_pair(C, gamma), min_sample, maj_sample);
