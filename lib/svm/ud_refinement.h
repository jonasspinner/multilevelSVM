...
template <class T> class ud_refinement : public svm_refinement<T> {
  public:
    ud_refinement(graph_hierarchy &min_hierarchy, graph_hierarchy &maj_hierarchy, const svm_result<T> &initial_result,
  ...
  private:
    svm_result<T> train_refinement(svm_solver<T> &solver, const svm_data &min_sample, const svm_data &maj_sample,
