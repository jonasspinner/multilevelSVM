...
class svm_solver_factory {
  public:
    template <class T> static std::unique_ptr<svm_solver<T>> create(const svm_instance &instance);
};

template <> inline std::unique_ptr<svm_solver<svm_model>> svm_solver_factory::create(const svm_instance &instance) {
    return std::make_unique<svm_solver_libsvm>(instance);
}

template <> inline std::unique_ptr<svm_solver<SVC>> svm_solver_factory::create(const svm_instance &instance) {
