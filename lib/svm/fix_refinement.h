...
template <class T> class fix_refinement : public svm_refinement<T> {
  public:
    fix_refinement(graph_hierarchy &min_hierarchy, graph_hierarchy &maj_hierarchy, const svm_result<T> &initial_result,
    ...
    static svm_result<T> train_fix(svm_solver<T> &solver, const svm_data &min_sample, const svm_data &maj_sample,
                                   float C, float gamma);
