...
class svm_io {
  public:
    ...
    template <typename T> static std::vector<T> take_sample(const std::vector<T> &data, float percentage);
};

template <typename T> std::vector<T> svm_io::take_sample(const std::vector<T> &data, float percentage) {
    std::vector<T> sample;
