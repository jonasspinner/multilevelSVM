...
void edge_ratings::compute_algdist(graph_access &G, std::vector<float> &dist) {
    ...
    for (auto node : G.nodes()) {
        forall_out_edges(G, e, node) { dist[e] += 0.0001; }
        endfor
    }
}
...
void edge_ratings::rate_pseudogeom(graph_access &G) {
    for (auto n : G.nodes()) {
        ...
        forall_out_edges(G, e, n) {
            ...
            EdgeRatingType rating =
                random_term * edgeWeight *
                (1.0 / (double)sqrt((double)targetWeight) + 1.0 / (double)sqrt((double)sourceWeight));
            G.setEdgeRating(e, rating);
        }
        endfor
    }
...
void edge_ratings::rate_separator_r2(graph_access &G) {
    for (auto node : G.nodes()) {
        forall_out_edges(G, e, node) {
            ...
            EdgeRatingType rating = 1.0 / (G.getNodeDegree(node) * G.getNodeDegree(target) * G.getNodeWeight(node) *
                                           G.getNodeWeight(target));
            G.setEdgeRating(e, rating);
        }
        endfor
    }
...
void edge_ratings::rate_separator_r7(graph_access &G) {
    for (auto node : G.nodes()) {
        forall_out_edges(G, e, node) {
            ...
            EdgeRatingType rating =
                G.getEdgeWeight(e) * 1.0 /
                (G.getNodeDegree(node) * G.getNodeDegree(target) * G.getNodeWeight(node) * G.getNodeWeight(target));
            G.setEdgeRating(e, rating);
        }
        endfor
    }
...
void edge_ratings::rate_separator_r8(graph_access &G) {
    for (auto node : G.nodes()) {
        forall_out_edges(G, e, node) {
            ...
            EdgeRatingType rating = G.getEdgeWeight(e) * 1.0 * (G.getNodeDegree(node) * G.getNodeDegree(target)) /
                                    (G.getNodeWeight(node) * G.getNodeWeight(target));
            G.setEdgeRating(e, rating);
        }
        endfor
